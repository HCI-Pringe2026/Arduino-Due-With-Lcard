...
class WaveformCanvas(FigureCanvas):
    ...
    def update_plot(self, a1, f1, p1_deg, a2, f2, p2_deg, dc, sr, gain_max):
        ...
        # Envelope: A(t) = |1 / sin(π(f2-f1)t + π/2)|, clamped to gain_max
        ...
        sines = a1 * np.sin(2 * np.pi * f1 * t + p1) + \
                a2 * np.sin(2 * np.pi * f2 * t + p2)

        combined = np.clip(dc + envelope * sines, 0, 3.3)

        t_ms = t * 1000

        # Plot individual (unscaled) sines faintly for reference
        self.ax.plot(t_ms, dc + a1 * np.sin(2*np.pi*f1*t + p1),
                     color=COLORS["sine1"], lw=0.8, alpha=0.35, label="Sine 1 (raw)")
        self.ax.plot(t_ms, dc + a2 * np.sin(2*np.pi*f2*t + p2),
                     color=COLORS["sine2"], lw=0.8, alpha=0.35, label="Sine 2 (raw)")
        # Envelope bounds
        env_top = np.clip(dc + envelope * (np.abs(a1) + np.abs(a2)), 0, 3.3)
        env_bot = np.clip(dc - envelope * (np.abs(a1) + np.abs(a2)), 0, 3.3)
        self.ax.fill_between(t_ms, env_bot, env_top,
                             color=COLORS["combined"], alpha=0.07)
        self.ax.plot(t_ms, env_top, color=COLORS["combined"],
                     lw=0.6, alpha=0.4, linestyle="--")
        self.ax.plot(t_ms, env_bot, color=COLORS["combined"],
                     lw=0.6, alpha=0.4, linestyle="--")
        # Combined output
        self.ax.plot(t_ms, combined, color=COLORS["combined"],
                     lw=1.6, label="Combined (envelope)")
        ...
        self.ax.set_title(
            f"peak {peak:.3f} V  |  trough {trough:.3f} V  |  gain cap ×{gain_max:.1f}",
...
class MainWindow(QMainWindow):
    ...
    def _build_global_box(self) -> QGroupBox:
        ...
        for ctrl in (self.dc_offset, self.sample_rate, self.gain_max):
            ctrl.valueChanged.connect(self._param_changed)
    ...
    def _check_clipping(self, p: dict) -> str:
        # Worst-case: envelope at gain_max, both sines peak together
        gain = p["gain_max"]
        worst_case = p["dc"] + gain * (p["a1"] + p["a2"])
        worst_low  = p["dc"] - gain * (p["a1"] + p["a2"])
        warnings = []
        if worst_case > 3.3:
            warnings.append(f"⚠ peak may clip up to +{worst_case:.3f} V (clamped)")
        if worst_low < 0:
            warnings.append(f"⚠ trough may clip down to {worst_low:.3f} V (clamped)")
    ...
    def _refresh_preview(self):
        ...
        # Update stats using same envelope math
        ...
        sines = (p["a1"] * np.sin(2*np.pi*p["f1"]*t + np.radians(p["p1"])) +
                 p["a2"] * np.sin(2*np.pi*p["f2"]*t + np.radians(p["p2"])))
        s = np.clip(p["dc"] + env * sines, 0, 3.3)
